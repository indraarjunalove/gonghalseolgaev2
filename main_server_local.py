...
DEPLOY_ENV = os.environ.get('DEPLOY_ENV', 'LOCAL')  # 'LOCAL' 또는 'RENDER'
...
def trigger_retrain():
    ...
    retrain.py를 별도 프로세스로 호출 → 기존 데이터 + 실측 로그 병합 학습.
    Render에선 호출 안 됨 (AUTO_RETRAIN_ON_STOP=False).
    """
    if not AUTO_RETRAIN_ON_STOP:
        print("[재학습] 비활성 환경 (RENDER) - 건너뜀")
        return False
    if not os.path.exists('retrain.py'):
        print("[재학습] retrain.py 없음 - 건너뜀")
        return False
    try:
